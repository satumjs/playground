...
use((sys, microApps, next) => {
  ...
  sys.set(MidwareName.code, (code, file) => {
    ...
    if (ext === FileType.CSS) {
      const { assetPublicPath = entry } =
        microApps.find((item) => item.fileExtNameMap[file]) || {};
      const matchs = code.match(/url\([^\)]+\)/gi);
      if (matchs) {
        matchs.forEach((s) => {
          const paths = s.match(/\(([^\)]+)\)/);
          ...
          if (url && url.charAt(0) === '.') {
            code = code.replace(
              s,
              s.replace(url, assetPublicPath + url.replace(/^\.{1,2}\//, ''))
            );
          }
        });
      }
...
use(simpleSandboxMidware, {
  winVariable(key, fakeWin, win) {
    ...
    if (key === 'getComputedStyle') {
      const orginGetComputedStyle = win[key].bind(win);
      return (...args) => {
        let el = args.shift();
        el = el instanceof Document ? el.body : el;
